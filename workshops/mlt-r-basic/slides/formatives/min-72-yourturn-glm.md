...
`workflow()` (recipe + spec). Score both on the held-out test set with
`roc_auc(..., event_level = "second")`.

**Confirm:** do the two routes give the **same ROC-AUC**? Say in one sentence why.

## Expected answer

Yes — the held-out ROC-AUC is the **same** ("same model, more scaffolding"). The
`workflow` does not change the statistical model; it only **bundles** the recipe and
the spec so the *same* logistic regression is fit on the *same* preprocessed data.
The number is identical because the estimator is identical.

## Stretch (Davide)

If the AUC is the same, what does the recipe + workflow **buy** you over a bare
