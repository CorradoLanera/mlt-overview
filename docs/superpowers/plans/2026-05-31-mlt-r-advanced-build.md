...
The workshop was built on `build/mlt-r-advanced`. The code snippets below are the plan-time intent; the **as-built** artifacts diverged as follows (each for a documented reason — see the commits + spec §11 override). Trust the artifacts + this list over the literal snippets.

- **SHAP backend → `kernelshap` (CRAN, pure R), NOT `fastshap`.** `fastshap` was archived from CRAN on 2026-05-27 (GitHub-only → needs C++/Rtools at `renv::restore`). `kernelshap` is `NeedsCompilation: no`, so `renv.lock` is 100% CRAN. API everywhere: `kernelshap(model, X = rows, bg_X = background, pred_fun = function(object, X) predict(object, X, type = "prob")$.pred_died)`; `permshap()` for the logistic anchor. `vip`'s `pred_wrapper` (signature `function(object, newdata)`) closes over the workflow because `vip` unwraps it to the bare engine. Applies to steps 01/02/04, the deck, and the formatives. See the spec §11 SHAP-backend override.
- **Synthetic notes file → `data-raw/hf_notes.csv`, NOT `ercp_notes.csv`** (the latter was a stale `indo_rct`/post-ERCP name; the cohort is heart-failure).
- **ellmer (step 03):** the labeled cache is `assets/ellmer-cache.rds` (tracked), NOT `output/...` (gitignored, would never ship). The key is read from `here("..", "..", ".Renviron")` (workshop root — `.here` anchors `here()` to the step folder), guarded by `file.exists`. The offline-fallback label uses `cat()` (because `execute: message: false` suppresses `message()`).
- **targets capstone (step 04):** the bridge `explain_model(model, cohort)` computes the **permutation VIMP of the reloaded RF** (genuinely consumes the Basic model), not a refit-logistic SHAP; the plan's `explain_anchor` name is retired. `reload_model()` calls **unqualified** `extract_workflow()` (NOT `tidymodels::extract_workflow`, which is a non-exported hardhat generic). The `_targets/` store is gitignored (`/steps/**/_targets/`).
- **Your-turn echo gate:** every `*-solved` chunk uses `eval=params$solved, echo=params$solved` (the plan's `eval`-only form would render the solution as visible source in the blank student handout).
- **`requirements.R`:** a fail-fast one-at-a-time install loop (renv reporter bug workaround) + `renv::snapshot(type = "all", prompt = FALSE)` (no project code references the packages at pin time, so an implicit snapshot would pin nothing). `visNetwork` is in the lock (for `tar_visnetwork`).
- **Release caveat:** the committed `_targets/meta/meta` records input hashes. If a bundled file (`heart_failure.csv` / `final_fit.rds`) is re-exported at dist time, refresh the targets meta, or the first `tar_make()` on a fresh clone may appear to skip rather than rebuild.
